...
import 'package:stormy_i18n_generator/src/generator/scaffold_generator.dart';
import 'package:stormy_i18n_generator/src/generator/dart_parser.dart';
import 'package:stormy_i18n_generator/src/generator/arb_generator.dart';
import 'package:stormy_i18n_generator/src/utils/logger.dart';
...
Future<void> run(List<String> args) async {
  final parser = ArgParser()
    ..addOption('flutter', help: 'Flutter executable for gen-l10n')
    ..addCommand(
      'init',
      ArgParser()..addFlag(
        'force',
  ...
  final sdkFlutter = path.normalize(
    path.join(
  ...
  final flutter =
      ...
      (File(sdkFlutter).existsSync() ? sdkFlutter : 'flutter');
  ...
  if (results.command?.name == 'init') {
    ...
    await _runGenerate(config, flutter);
    return;
  }
  ...
  if (results.command?.name == 'watch') {
    ...
    if (!file.existsSync()) {
      I18nLogger.info(
        '❌ 未找到 stormy_i18n.yaml 配置文件，请先运行: dart run stormy_i18n_generator init',
        '❌ Config file stormy_i18n.yaml not found, please run: dart run stormy_i18n_generator init first',
      );
      exit(1);
    }
    ...
    final queue = SerialGenerationQueue(() async {
      ...
      if (config.enabled) await _runGenerate(config, flutter);
    });
...
Future<void> _runGenerate(StormyI18nConfig config, String flutter) async {
...
void _generateExampleFile(StormyI18nConfig config) {
  ...
  final Map<String, Map<String, String>> examples = {
    ...
    'helloWorldOn': {
      ...
      'zh': "'{date}:{time} 您好, {name}, {gender, select, male{他} female{她} other{他/她}}已经有{count}个未读消息'",
      'en': "' {name} Hello World on {date}:{time}, {gender, select, male{he} female{she} other{they}} have {count} unread {count, plural, =1{message} other{messages}}'",
      'ja': "'{date}:{time} こんにちは {name}, {gender, select, male{彼} female{彼女} other{彼ら}} は {count} 件の未読メッセージがあります'",
