...
mixin StorageOperations {
  ...
  Object _prefixedKey(Object key) {
    validateStorageKey(key);
    return key is String ? storageConfig?.prefixedKey(key) ?? key : key;
  }

  // ============== Core KV ==============

  /// 保存任意值（支持过期时间）
  Future<void> set(Object key, dynamic value, {Duration? expiresIn}) async {
  ...
  /// 获取值（自动检查过期）
  T? get<T>(Object key, {T Function(dynamic)? decoder}) {
    ensureStorageInitialized();
    final prefixedKey = _prefixedKey(key);
  ...
  Future<void> setString(Object key, String value) async => set(key, value);
  String? getString(Object key) => get<String>(key);
  Future<void> setInt(Object key, int value) async => set(key, value);
  int? getInt(Object key) => get<int>(key);
  Future<void> setBool(Object key, bool value) async => set(key, value);
  bool? getBool(Object key) => get<bool>(key);
  Future<void> setDouble(Object key, double value) async => set(key, value);
  double? getDouble(Object key) => get<double>(key);
  ...
  Map<String, dynamic>? getJson(Object key) {
    final value = get<Map<dynamic, dynamic>>(key);
    if (value == null) return null;
    return value.cast<String, dynamic>();
  }

  Future<void> setJsonWithExpiry(
    Object key,
    Map<String, dynamic> value,
    Duration expiresIn,
  ) async {
  ...
  dynamic getObject(Object key) => get(key);

  // ============== 生命周期与清理 ==============

  Future<void> remove(Object key) async {
    ensureStorageInitialized();
    await storageEngine.remove(
      _prefixedKey(key),
      bucketName: currentBucketName,
    );
  }

  Future<void> removeMany(List<Object> keys) async {
    ensureStorageInitialized();
    final formattedKeys = keys.map(_prefixedKey).toList();
    await storageEngine.removeMany(
      formattedKeys,
      bucketName: currentBucketName,
    );
  }
  ...
  bool containsKey(Object key) {
    ensureStorageInitialized();
    return storageEngine.containsKey(
      _prefixedKey(key),
      bucketName: currentBucketName,
    );
  }

  List<Object> getKeys({bool excludeExpired = true}) {
    ensureStorageInitialized();
    ...
    for (final rawKey in allKeys) {
      ...
      // 过滤系统内部管理的元信息键
      final internalKeyObj = rawKey is String
          ? rawKey.substring(prefix.length)
          : rawKey;
      if (internalKeyObj is String &&
          (internalKeyObj.startsWith('\$list:') ||
              internalKeyObj.startsWith('\$meta:'))) {
        continue;
      }
  ...
  Duration? getExpiresIn(Object key) {
    ensureStorageInitialized();
    final raw = storageEngine.get(
      _prefixedKey(key),
      bucketName: currentBucketName,
