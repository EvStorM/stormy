...
class StormyStorage with StorageOperations {
  ...
  Future<void> initialize({
    ...
    if (_isInitialized) {
      ...
      throw StateError('Storage 已初始化；更换配置前请先 close()');
    }
    final candidate = engine ?? config.engine ?? HiveStorageEngine();
    try {
      await candidate.init(registerAdapters: registerAdapters);
      for (final bucketConfig in config.buckets) {
        await candidate.openBucket(bucketConfig);
      }
