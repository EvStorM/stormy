...
import 'package:stormy_ui/config/accessor/ui_config.dart';
...
ClassicHeader buildDefaultHeader() {
  ...
  final theme = StormyUiConfig.theme;
  ...
  return ClassicHeader(
    ...
    readyText:
        refreshConfig.stateTexts?.ready ?? textConfig.loading ?? '加载中...',
    processingText:
        refreshConfig.stateTexts?.processing ?? textConfig.loading ?? '加载中...',
    armedText:
        refreshConfig.stateTexts?.armed ?? textConfig.releaseToLoad ?? '释放加载',
    dragText: refreshConfig.stateTexts?.drag ?? textConfig.pullToLoad ?? '下拉加载',
    processedText:
        refreshConfig.stateTexts?.processed ??
        textConfig.loadSuccessMessage ??
        '加载完成',
    failedText:
        refreshConfig.stateTexts?.failed ?? textConfig.loadFailed ?? '加载失败',
    messageText: '',
