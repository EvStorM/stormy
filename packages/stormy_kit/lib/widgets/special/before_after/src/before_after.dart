...
class BeforeAfter extends StatefulWidget {
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    if (trackColor is Color) {
      properties.add(ColorProperty('trackColor', trackColor as Color?));
    } else if (trackColor is Gradient) {
      properties.add(
        DiagnosticsProperty<Gradient>('trackColor', trackColor as Gradient?),
      );
    } else {
      properties.add(DiagnosticsProperty<Object?>('trackColor', trackColor));
    }
    properties.add(
      FlagProperty(
        'hideThumb',
...
class _BeforeAfterState extends State<BeforeAfter>
  ...
  Widget build(BuildContext context) {
    ...
    final onHorizontalChanged = widget.onValueChanged != null
        ? _handleValueChanged
        : null;
    ...
    final effectiveOverlayColor =
        ...
        (widget.trackColor is Color
            ? (widget.trackColor as Color).withOpacity(0.12)
            : null) ??
        WidgetStateProperty.resolveAs<Color?>(
          beforeAfterTheme.overlayColor,
          states,
        ) ??
      ...
      child: MouseRegion(
        ...
        child: FocusableActionDetector(
          ...
          child: TwoDirectionalSlider(
            ...
            verticalDivisions: isXAxis
                ? widget.thumbDivisions
                : widget.divisions,
            horizontalDivisions: isXAxis
                ? widget.divisions
                : widget.thumbDivisions,
            onVerticalChangeStart: _handleDragStart,
            onVerticalChanged: isXAxis
                ? onVerticalChanged
                : onHorizontalChanged,
            onVerticalChangeEnd: _handleDragEnd,
            onHorizontalChangeStart: _handleDragStart,
            onHorizontalChanged: isXAxis
                ? onHorizontalChanged
                : onVerticalChanged,
...
class Hide extends StatelessWidget {
  ...
  const Hide({super.key, required this.child});
...
class _BeforeAfterDefaultsM2 extends BeforeAfterTheme {
  ...
  @override
  Object get trackColor => _colors.primary;
  ...
  BoxDecoration get thumbDecoration {
    return BoxDecoration(
      color: trackColor is Color ? trackColor as Color : null,
      gradient: trackColor is Gradient ? trackColor as Gradient : null,
  ...
  Color get overlayColor {
    return WidgetStateColor.resolveWith((states) {
      if (states.contains(WidgetState.hovered)) {
        return _colors.primary.withAlpha(8);
      }
      if (states.contains(WidgetState.focused)) {
        return _colors.primary.withAlpha(12);
      }
