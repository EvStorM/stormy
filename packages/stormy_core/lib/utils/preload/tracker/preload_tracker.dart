...
class PreloadTracker {
  ...
  void failAll(Object error) {
    for (final context in _contexts) {
      if (!context.isFinished) context.markFailed(error);
    }
    _scheduleProgressUpdate();
  }
  ...
  PreloadProgress _computeProgress() {
    ...
    final cancelledTasks = _contexts
        .where((c) => c.state == PreloadTaskState.cancelled)
        .length;
    final finishedTasks = completedTasks + failedTasks + cancelledTasks;
    final progress = _contexts.isNotEmpty
        ? finishedTasks / _contexts.length
        : 1.0;
